...
import {
  ...
  PersonIcon,
  RocketIcon,
} from "../Icons/Icons"; // Fixed relative path

import { SidebarResponsive } from "../Sidebar/Sidebar"; // Fixed relative path
import React from "react";
import { NavLink } from "react-router-dom";
import routes from "../../routes"; // Fixed relative path

export default function AuthNavbar(props) {
  const { logoText, secondary, ...rest } = props;
  const { colorMode } = useColorMode();

  // Chakra color mode values (hooks called at top)
  const bgButton = useColorModeValue("white", "navy.900");
  const colorButton = useColorModeValue("gray.700", "white");
  const hamburgerBaseColor = useColorModeValue("gray.700", "white");
  ...
  let hamburgerColor = { base: hamburgerBaseColor, md: "white" };
